...
api = NinjaExtraAPI()
@api_controller('/league', tags=['League'], permissions=[])
class LeagueApi:
    @http_post('/', response=LeagueOut)
...
@api_controller('/team', tags=['Team'], permissions=[])
class TeamApi:
    @http_post('/', response=TeamOut)
    ...
    @http_get('/', response=List[TeamOut])
    def list_team(request):
        """축구 팀 리스트"""
        return Team.objects.all()

    @http_get('/{team_id}', response=TeamOut)
